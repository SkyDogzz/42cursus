...
/*   Updated: 2024/12/23 17:52:37 by tstephan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_vertical_lines(t_wrapper *wrapper, int x, int y, t_2vec start)
{
	...
	end.x = wrapper->map->content[(y + 1) * wrapper->map->dims.width + x].pos.x
		* wrapper->map->zoom + WINDOW_WIDTH / 2 + wrapper->map->offset.x;
	end.y = wrapper->map->content[(y + 1) * wrapper->map->dims.width + x].pos.y
		* wrapper->map->zoom + WINDOW_HEIGHT / 2 + wrapper->map->offset.y;
	color.start = wrapper->map->content[y * wrapper->map->dims.width + x].color;
	color.end = wrapper->map->content[(y + 1) * wrapper->map->dims.width
		+ x].color;
...
void	draw_horizontal_lines(t_wrapper *wrapper, int x, int y, t_2vec start)
{
	...
	end.x = wrapper->map->content[y * wrapper->map->dims.width + x + 1].pos.x
		* wrapper->map->zoom + WINDOW_WIDTH / 2 + wrapper->map->offset.x;
	end.y = wrapper->map->content[y * wrapper->map->dims.width + x + 1].pos.y
		* wrapper->map->zoom + WINDOW_HEIGHT / 2 + wrapper->map->offset.y;
	color.start = wrapper->map->content[y * wrapper->map->dims.width + x].color;
	color.end = wrapper->map->content[y * wrapper->map->dims.width + x
		+ 1].color;
	draw_line(start, end, color, *wrapper);
}
